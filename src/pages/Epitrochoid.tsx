...
function Epitrochoid() {
  const [params, setParams] = useState<EpitrochoidParams>({
    R: 100,
    r: 30,
    d: 60,
    amplitude: 150,
  ...
  const generatePath = useCallback(
    (
      ...
      const theoreticalMax = Math.abs(R + r) + Math.abs(d);
      ...
      for (let i = 0; i <= steps; i++) {
        ...
        const x =
          (R + r) * Math.cos(theta) -
          d * Math.cos(((R + r) / r) * theta + internalPhase);
        const y =
          (R + r) * Math.sin(theta) -
          d * Math.sin(((R + r) / r) * theta + internalPhase);
      ...
      rawPoints.forEach((p, i) => {
        const nx = (p.x / theoreticalMax) * amplitude + 250;
        const ny = (p.y / theoreticalMax) * amplitude + 250;

        if (i === 0) {
          path += `M ${nx.toFixed(2)} ${ny.toFixed(2)}`;
        } else {
          path += ` L ${nx.toFixed(2)} ${ny.toFixed(2)}`;
        }
      });
  ...
  const updateParam = useCallback((key: keyof EpitrochoidParams, value: number | string) => {
  ...
  const downloadSVG = useCallback(() => {
    ...
    link.href = url;
    link.download = `epitrochoid-curve-${Date.now()}.svg`;
  ...
  return (
    <div className="min-h-screen bg-black overflow-x-hidden">
      <Helmet>
        <title>Designer | Epitrochoid</title>
      </Helmet>
      <Topbar />
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              ...
              <div className="bg-white/[0.02] backdrop-blur-sm rounded-2xl p-6 border border-white/[0.08]">
                <div className="flex justify-center">
                  <div className="relative w-full max-w-[500px] mx-auto bg-black rounded-xl p-4 border border-white/[0.05]">
                    ...
                    <div className="absolute bottom-2 right-2 text-white text-xs font-mono space-y-1 text-right">
              ...
              <div className="bg-white/[0.02] backdrop-blur-sm rounded-2xl p-6 border border-white/[0.08]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                  <div className="text-center">
                    <div className="text-gray-400 font-light">R (Fixed)</div>
                    <div className="text-white font-medium">{params.R}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-gray-400 font-light">r (Rolling)</div>
                    <div className="text-white font-medium">{params.r}</div>
                  </div>
                  <div className="text-center">
                    <div className="text-gray-400 font-light">d (Offset)</div>
                    <div className="text-white font-medium">{params.d}</div>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/[0.02] backdrop-blur-sm rounded-2xl p-6 border border-white/[0.08]">
                <div className="flex items-center gap-3 mb-6">
                  <Settings className="w-5 h-5 text-gray-400" />
                  <h3 className="text-xl font-light text-white">Offset</h3>
                </div>
                
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-light text-gray-300 mb-2">
                      d (Pen Offset): {params.d}
                    </label>
                    <input
                      type="range"
                      min="0"
